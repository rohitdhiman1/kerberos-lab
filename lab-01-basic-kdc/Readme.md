...
# Lab 01: Basic Kerberos KDC Setup

This lab provides a foundational Kerberos environment with a KDC and client for hands-on learning.

## What You'll Learn

- Kerberos architecture and core concepts
- KDC database initialization
- Principal management with kadmin
- Ticket lifecycle (kinit, klist, kdestroy)
...
```
┌─────────────────┐         ┌──────────────────┐
│  kerberos-kdc   │◄────────┤ kerberos-client  │
│  (KDC Server)   │         │  (Test Client)   │
│                 │         │                  │
│  - krb5kdc      │         │  - kinit         │
│  - kadmind      │         │  - klist         │
│  Port: 88, 749  │         │  - kadmin        │
...
- `KRB_REALM`: Your Kerberos realm (e.g., `DEV.LOCAL`)
- `KRB_MASTER_PASSWORD`: Master password for the KDC database
- `KRB_ADMIN_PASSWORD`: Password for the admin principal

#### 1.2 Prepare Local Volumes

Before running Docker Compose, ensure the host directories for persistent data exist:
...
#### 1.3 Build and Start the Services

The `--build` flag ensures the Dockerfile is processed, and `-d` runs containers in background:
...
#### 1.4 Monitor Initialization (First Run)
